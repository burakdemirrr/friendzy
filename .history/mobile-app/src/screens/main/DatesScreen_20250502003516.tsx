...
export default function DatesScreen() {
  ...
  const handleDatePress = (invitation: DateInvitation) => {
    if (invitation.status === 'accepted') {
      const dateTime = new Date(invitation.date_time);
      const now = new Date();
      
      // If the date has arrived or passed
      if (dateTime <= now) {
        navigation.navigate('DateChallenges', { dateId: invitation.id });
      }
    }
  };
            ...
            {invitations.map((invitation) => (
              ...
              >
                <StyledTouchableOpacity
                  onPress={() => handleDatePress(invitation)}
